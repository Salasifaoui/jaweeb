 ...
 */
function useUpdateVerification(props?: MutationOptions<Models.Token, unknown, TRequest, unknown>) {
  ...
  const mutation = useMutation<Models.Token, unknown, TRequest, unknown>({
    ...
    mutationFn: async request => {
      return await account.updateVerification(request.userId, request.secret)
    },
